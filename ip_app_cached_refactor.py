...
with col_a:
    _run_main = st.button("▶ Run Scenario / Refresh", help="Compute forecasts with current inputs")
if _run_main:
    st.session_state["run_ok"] = True
    st.rerun()
# ===== End Global Run Button =====
...
if uploaded_json is not None:
    ...
    if st.session_state["_scenario_loaded_hash"] != file_hash:
        try:
            ...
            # ---- Scalars ----
            for _k in ["start_date", "end_date", "buffer_percent"]:
                if _k in payload:
                    st.session_state[_k] = payload[_k]
