...
st.sidebar.subheader("Kit entry mode")
st.session_state["per_kit_qty_mode"] = st.sidebar.checkbox(
    "Use per-kit Qty (ignore Active/Placebo)",
    value=st.session_state["per_kit_qty_mode"],
    help="Visit table uses a single Qty column; each listed kit gets Qty at that visit offset."
)
# refresh local copy after the checkbox
per_kit_qty_mode = st.session_state["per_kit_qty_mode"]

st.header("Visit Schedule")
st.caption("Turn ON **Per-kit quantities** in the sidebar to switch to a single `Qty` column. "
           "Each kit listed on the row receives `Qty` bottles at that visit's offset. "
           "Active vs Placebo is inferred from kit names (strings like 'PBO'/'Placebo' → Placebo).")


# --- Blinded parity check (Active vs Placebo) ---
try:
    if "visit_df" in locals():
        # Filter to blinded rows (exclude OLE kits)
        mask_blinded = ~visit_df["Kit_Type"].astype(str).str.contains("_OLE", case=False, na=False)
        blinded = visit_df.loc[mask_blinded].copy()

        # Coerce numeric
        for col in ["Bottles_Active", "Bottles_Placebo", "Month_Offset"]:
            if col in blinded.columns:
                blinded[col] = pd.to_numeric(blinded[col], errors="coerce").fillna(0)

        st.subheader("Blinded Arm Parity Check")
        if len(blinded) == 0:
            st.info("No blinded visits found (or Visit table is empty).")
        else:
            # Sum bottles per arm by Month_Offset
            sums = blinded.groupby("Month_Offset")[["Bottles_Active","Bottles_Placebo"]].sum(numeric_only=True)

            # Find offsets where totals don't match
            mismatches = sums[sums["Bottles_Active"] != sums["Bottles_Placebo"]]

            if mismatches.empty:
                st.success("✅ Active and Placebo bottles match at all blinded visits.")
            else:
                st.error("⚠️ Mismatched Active vs Placebo bottles at some offsets:")
                st.dataframe(mismatches)

            # Kit presence parity by offset
            kits_by_offset = {}
            for off, sub in blinded.groupby("Month_Offset"):
                kits_active = set()
                kits_placebo = set()
                for _, r in sub.iterrows():
                    kits = [k.strip() for k in str(r.get("Kit_Type", "")).split(",") if k.strip()]
                    # active kits (non-placebo strings) if active bottle count > 0
                    if float(r.get("Bottles_Active", 0)) > 0:
                        kits_active.update(k for k in kits if ("PBO" not in k.upper() and "PLACEBO" not in k.upper()))
                    # placebo kits if placebo bottle count > 0
                    if float(r.get("Bottles_Placebo", 0)) > 0:
                        kits_placebo.update(k for k in kits if ("PBO" in k.upper() or "PLACEBO" in k.upper()))
                if kits_active != kits_placebo:
                    try:
                        off_int = int(off)
                    except Exception:
                        off_int = off
                    kits_by_offset[off_int] = {
                        "Active kits": sorted(kits_active),
                        "Placebo kits": sorted(kits_placebo),
                    }

            if kits_by_offset:
                st.warning("⚠️ Kit type mismatch between Active and Placebo at some offsets:")
                st.json(kits_by_offset)
    else:
        st.info("Visit table not found for parity check.")
except Exception as e:
    st.warning(f"Parity check skipped: {e}")
