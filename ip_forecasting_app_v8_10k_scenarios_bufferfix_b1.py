...
# --- Scenario save/load helpers (Option 1) ---
import json
...
def serialize_config() -> Dict[str, Any]:
    return {
        ...
        "Active_ratio":   float(st.session_state.get("Active_ratio", 1.0)),
        "Placebo_ratio":  float(st.session_state.get("Placebo_ratio", 1.0)),
        "buffer_percent": float(st.session_state.get("buffer_percent", 0.15)),
        "target_enrollment_global": int(st.session_state.get("target_enrollment_global", 0)),
        "countries_df": _df_to_records(st.session_state.get("countries_df")),
        "visit_df":     _df_to_records(st.session_state.get("visit_df")),
        "sku_df":       _df_to_records(st.session_state.get("sku_df")),
        "lots_df":      _df_to_records(st.session_state.get("lots_df")),
        "auto_generate_sites": bool(st.session_state.get("auto_generate_sites", True)),
        "sites_active_df": _df_to_records(st.session_state.get("sites_active_df")),
    }

def restore_config(payload: Dict[str, Any]) -> None:
    import pandas as pd
    # dates
    try: st.session_state["start_date"] = pd.to_datetime(payload.get("start_date")).date()
    except Exception: pass
    try: st.session_state["end_date"]   = pd.to_datetime(payload.get("end_date")).date()
    except Exception: pass
    # numbers
    for k, cast in [
        ("Active_ratio", float),
        ...
        try: st.session_state[k] = cast(payload.get(k, st.session_state.get(k, 0)))
        except Exception: pass
    # Normalize buffer_percent: allow legacy 0-1 or new 1-100
    try:
        bp = st.session_state.get("buffer_percent", 15)
        # if given as 0-1, convert to 1-100
        if 0 <= bp <= 1:
            bp = int(round(bp * 100))
        else:
            bp = int(round(bp))
        # clamp
        bp = max(0, min(100, bp))
        st.session_state["buffer_percent"] = bp
    except Exception:
        st.session_state["buffer_percent"] = 15
    ...
    # flags
    if "auto_generate_sites" in payload:
        st.session_state["auto_generate_sites"] = bool(payload["auto_generate_sites"])
...
st.session_state.setdefault('start_date', datetime(2026, 1, 1).date())
st.session_state.setdefault('end_date', datetime(2027, 12, 31).date())
...
if uploaded is not None:
    try:
        ...
        restore_config(payload)
        st.sidebar.success('Scenario loaded. Rerunning...')
        st.experimental_rerun()
    except Exception as e:
        st.sidebar.error(f'Could not load scenario: {e}')

scenario_name = st.sidebar.text_input('Scenario name', value='my_scenario')
if st.sidebar.button('💾 Save scenario to JSON'):
    blob = json.dumps(serialize_config(), indent=2).encode('utf-8')
...
st.session_state.setdefault('target_enrollment_global', min(sum_country_targets, 1000))
target_enrollment_global = st.sidebar.number_input(
    'Target total enrollment (global)',
    min_value=0, max_value=1000,
    value=st.session_state['target_enrollment_global'],
    step=1, key='target_enrollment_global'
)

# ---------- Study Arms (ratios only) ----------
st.sidebar.header("Study Arms & Randomization")
st.session_state.setdefault('Active_ratio', 1.0)
st.session_state.setdefault('Placebo_ratio', 1.0)
randomization = {
    'Active': st.sidebar.number_input('Randomization Ratio - Active', min_value=0.0, value=st.session_state['Active_ratio'], step=0.05, format='%.4f', key='Active_ratio'),
    'Placebo': st.sidebar.number_input('Randomization Ratio - Placebo', min_value=0.0, value=st.session_state['Placebo_ratio'], step=0.05, format='%.4f', key='Placebo_ratio'),
}
...
st.session_state.setdefault('buffer_percent', 15)
st.sidebar.markdown('**Buffer applies to demand when computing bottles**')
# Use the slider's return value; don't assign to the same key explicitly to avoid API exception
buffer_percent_val = st.sidebar.slider('Buffer (%)', 0, 100, st.session_state['buffer_percent'], step=1, key='buffer_percent')
# Warning if 0% buffer
if buffer_percent_val == 0:
    st.sidebar.warning('Buffer is 0%. You are forecasting with no safety margin.', icon='⚠️')
# Use fraction for calculations
buffer_fraction = buffer_percent_val / 100.0
...
st.session_state.setdefault('auto_generate_sites', True)
auto_generate = st.checkbox('Auto-generate from per-country ramp / custom sequence', value=st.session_state['auto_generate_sites'], key='auto_generate_sites')
...
visit_df["Month_Offset"] = (visit_df["Day_of_Visit"] // 30).astype(int)
...
for arm in randomization:
    for m, month_lbl in enumerate(month_labels):
        ...
        bottles_by_arm[arm][m] = math.ceil(total_bottles * (1.0 + buffer_fraction)) if total_bottles > 0 else 0.0
...
for arm in randomization:
    for k in range(max_offset_local + 1):
        ...
        for _, r in rows.iterrows():
            kits = parse_kits(r.get("Kit_Type", ""))
            total_kits += max(1, len(kits)) if len(kits) > 0 else 0
            for kt in kits:
...
for arm in randomization:
    for m, month_lbl in enumerate(study_months_str):
        ...
        for kt, val in month_total_by_kit.items():
            demand_kit_monthly.loc[pd.to_datetime(month_lbl), kt] += math.ceil(val * (1.0 + buffer_fraction))
        ...
        for lg in label_groups_sorted:
            for kt2 in kit_types_all:
                ...
                demand_label_kit.loc[row_idx, (lg, kt2)] = math.ceil(raw_val * (1.0 + buffer_fraction)) if raw_val > 0 else 0.0
