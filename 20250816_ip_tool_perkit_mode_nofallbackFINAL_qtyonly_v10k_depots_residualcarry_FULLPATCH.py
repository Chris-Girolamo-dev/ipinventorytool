    ...
    try:
        ...
        if kr is not None:
            ...
            if not kr_df.empty:
                ...
                if not kr_df.empty:
                    st.session_state["kit_rand_df"] = kr_df.copy()
                    return True
        ...
        if kits and "kit_rand_df" not in st.session_state:
            ...
            st.session_state["kit_rand_df"] = tmp.copy()
            return True
...
# --- Per-kit Qty mode: always defined early ---
try:
    _ = st.session_state  # ensure session exists
except Exception:
    pass
if "per_kit_qty_mode" not in st.session_state:
    st.session_state["per_kit_qty_mode"] = True
# local copy that will always exist before first use
per_kit_qty_mode = st.session_state["per_kit_qty_mode"]
...
# ---- Kit entry mode (shows before Visit Schedule) ----

# Per-kit Qty mode is enforced; UI toggle removed
per_kit_qty_mode = True

st.header("Visit Schedule")
st.caption(
    "Per-kit quantities are enabled. Each kit listed on the row receives `Qty` bottles at that visit's offset. "
    "Active vs Placebo is inferred from kit names (strings like 'PBO'/'Placebo' → Placebo)."
)
st.caption("""
...
# Ensure Qty column exists/clean when using per-kit mode
try:
    if per_kit_qty_mode:
        if "Qty" not in st.session_state['visit_df'].columns:
            st.session_state['visit_df']["Qty"] = 1
        st.session_state['visit_df']["Qty"] = pd.to_numeric(st.session_state['visit_df']["Qty"], errors="coerce").fillna(1).astype(int)
except Exception:
    pass
...
with st.form('visit_form', clear_on_submit=False):
    # Only Qty is editable (Active/Placebo columns removed)
    column_config_visit = {
        ...
        'Month_Offset': st.column_config.NumberColumn('Month_Offset', disabled=True),
    }
    # ensure Qty column is present
    if "Qty" not in st.session_state['visit_df'].columns:
        st.session_state['visit_df']["Qty"] = 1
    st.session_state['visit_df']["Qty"] = pd.to_numeric(st.session_state['visit_df']["Qty"], errors="coerce").fillna(1).astype(int)

    # Hide Bottle columns if present
    for _c in ["Bottles_Active","Bottles_Placebo"]:
        if _c in st.session_state['visit_df'].columns:
            st.session_state['visit_df'].drop(columns=[_c], inplace=True)

    visit_df = st.data_editor(
        st.session_state['visit_df'][['Visit_Number','Day_of_Visit','Kit_Type','Qty','Month_Offset']],
        num_rows='dynamic',
        use_container_width=True,
        key='visit_editor',
        column_config=column_config_visit,
    )
    visit_save = st.form_submit_button('Save Visit Schedule')

if 'visit_save' in locals() and visit_save:
    # qty-only cleanup
    if "Qty" not in visit_df.columns:
        visit_df["Qty"] = 1
    visit_df["Qty"] = pd.to_numeric(visit_df["Qty"], errors="coerce").fillna(1).astype(int)
    drop_cols = [c for c in ['Bottles_Active','Bottles_Placebo'] if c in visit_df.columns]
    if drop_cols:
        visit_df.drop(columns=drop_cols, inplace=True)

    # per_kit_qty_mode cleanup
    if per_kit_qty_mode:
        if "Qty" not in visit_df.columns:
            visit_df["Qty"] = 1
        visit_df["Qty"] = pd.to_numeric(visit_df["Qty"], errors="coerce").fillna(1).astype(int)
        # Keep Bottles_Active/Placebo as zeros to avoid accidental double counting
        for _c in ['Bottles_Active','Bottles_Placebo']:
            if _c in visit_df.columns:
...
# ---- OLE helpers ----
def is_ole_kit(name: str) -> bool:
    try:
        return isinstance(name, str) and name.endswith("_OLE")
    except Exception:
        return False

def ole_base_names(ole_name: str):
    """Map 'ali_4mg_OLE' -> ('ali_4mg_Active','ali_4mg_Placebo')."""
    if not isinstance(ole_name, str) or "_OLE" not in ole_name:
        return None, None
    base = ole_name.replace("_Active_OLE","").replace("_Placebo_OLE","").replace("_OLE","")
    return f"{base}_Active", f"{base}_Placebo"
...
# ---------- Enrollment rounding (residual-carry) ----------
st.sidebar.subheader("Enrollment rounding")
use_residual_carry = st.sidebar.checkbox(
    "Use residual-carry rounding for enrollments",
    value=True,
    help=("Accumulate fractional enrollments month-to-month so small rates "
          "(e.g., 0.5/mo) generate subjects earlier when Visit 1 is at Day 0.")
)
# ---------------------------------------------------------
# Sync per-kit flag defined earlier
per_kit_qty_mode = True
...
offset_bottles_arr = {
    arm: np.array([
        0.0 if per_kit_qty_mode else float(visit_df.loc[visit_df["Month_Offset"] == k, f"Bottles_{arm}"].sum())
...
# ---------- Residual-carry rounding helper ----------
def residual_carry_round(values, method: str = "round", rng=None):
    """
    Accumulate fractional enrollments month-to-month so that small rates
    (e.g., 0.5 subjects/mo) generate subjects at the right time instead of
    slipping later when rounded per month.

    values : 1D iterable of floats (per-month enrollments)
    method : one of {"round","floor","ceil","poisson"}
    rng    : np.random.Generator for reproducible Poisson if needed

    Returns list of ints, same length as values.
    """
    if rng is None:
    ...
    for x in values:
        ...
        if m == "round":
            ...
            # Poisson doesn't combine with carry in a meaningful way; treat total as mean and reset.
            n = int(np.random.default_rng().poisson(max(0.0, total))) if rng is None else int(rng.poisson(max(0.0, total)))
            carry = 0.0
        else:
            n = int(np.floor(total + 0.5))
            carry = total - n
...
# ---------- Convert fractional monthly enrollments → integer subjects (with optional residual-carry) ----------
try:
    _rounding_needs_carry = (mc_subject_count in {"round", "floor", "ceil"})
except Exception:
    _rounding_needs_carry = True

if use_residual_carry and _rounding_needs_carry:
    for c in country_list:
        # Active
        vals = new_enroll_arm_country[c]["Active"]
        new_enroll_arm_country[c]["Active"] = residual_carry_round(vals, mc_subject_count, rng=rng)
        # Placebo
        vals = new_enroll_arm_country[c]["Placebo"]
        new_enroll_arm_country[c]["Placebo"] = residual_carry_round(vals, mc_subject_count, rng=rng)
# else: keep existing per-month rounding in _to_subjects inside the MC loop
# ---------------------------------------------------------------------------------------------


# Precompute: bottles per subject for (arm, offset, kit) — PER ROW (no whole-offset totals).
...
for k in range(max_offset_local + 1):
    ...
    for _, r in rows.iterrows():
        kits_all = [p.strip() for p in str(r.get("Kit_Type","")).split(",") if p.strip()]
        if per_kit_qty_mode:
            qty = float(r.get("Qty", 0) or 0.0)
            if qty <= 0:
                continue
            kits_active = [kt for kt in kits_all if not is_placebo_kit(kt)]
            kits_placebo = [kt for kt in kits_all if is_placebo_kit(kt)]
            for kt in kits_active:
                kit_bottles_ps["Active"][(k, kt)] = kit_bottles_ps["Active"].get((k, kt), 0.0) + qty
            for kt in kits_placebo:
                kit_bottles_ps["Placebo"][(k, kt)] = kit_bottles_ps["Placebo"].get((k, kt), 0.0) + qty
        else:
            b_act = float(r.get("Bottles_Active", 0.0) or 0.0)
            b_pbo = float(r.get("Bottles_Placebo", 0.0) or 0.0)
            if b_act > 0:
                for kt in [x for x in kits_all if not is_placebo_kit(x)]:
                    kit_bottles_ps["Active"][(k, kt)] = kit_bottles_ps["Active"].get((k, kt), 0.0) + b_act
            if b_pbo > 0:
                for kt in [x for x in kits_all if is_placebo_kit(x)]:
                    kit_bottles_ps["Placebo"][(k, kt)] = kit_bottles_ps["Placebo"].get((k, kt), 0.0) + b_pbo
...
# Build kit groups from Visit Schedule (exclude OLE from MC pools)
_all_kits = sorted({kit for s in st.session_state["visit_df"]["Kit_Type"].fillna("").astype(str)
                    for kit in parse_kits(s)})
active_base_kits   = [k for k in _all_kits if (not is_placebo_kit(k)) and (not is_ole_kit(k)) and k.endswith("_Active")]
placebo_base_kits  = [k for k in _all_kits if is_placebo_kit(k) and k.endswith("_Placebo")]
ole_kits           = [k for k in _all_kits if is_ole_kit(k)]

active_kits_order = active_base_kits
placebo_kits_order = placebo_base_kits
p_active = _probs_for_arm("Active", active_kits_order)
p_placebo = _probs_for_arm("Placebo", placebo_kits_order)

# ---- Build kit list directly from SKU Catalog (auto-includes Active, Placebo, OLE, etc.) ----
if "sku_df" in st.session_state:
    kit_types_all = sorted(
        st.session_state["sku_df"]["Kit_Type"]
        .dropna()
        .astype(str)
        .unique()
        .tolist()
...
# ---------- Depots: Country → Depot mapping ----------
st.sidebar.subheader("Depots")

_default_depot_map = pd.DataFrame(
    {"Country": country_list, "Depot": ["Default"] * len(country_list)}
)
if "depots_map_df" not in st.session_state or st.session_state["depots_map_df"].empty:
    st.session_state["depots_map_df"] = _default_depot_map.copy()

with st.expander("Depot mapping (Country → Depot)", expanded=False):
    depots_map_df = st.data_editor(
        st.session_state["depots_map_df"],
        use_container_width=True,
        num_rows="dynamic",
        column_config={
            "Country": st.column_config.TextColumn("Country", help="Match the Countries table"),
            "Depot": st.column_config.TextColumn("Depot", help="Shared depot name for this country")
        },
        key="depots_map_editor",
    )
    if st.button("Save Depot Mapping", type="primary"):
        depots_map_df["Country"] = depots_map_df["Country"].astype(str).str.strip()
        depots_map_df["Depot"] = depots_map_df["Depot"].astype(str).str.strip()
        depots_map_df = depots_map_df[depots_map_df["Country"] != ""]
        st.session_state["depots_map_df"] = depots_map_df.copy()
        st.session_state["run_ok"] = False
        st.success("Depot mapping saved. Click **Run Scenario / Refresh** to recompute.")

depots_map_df = st.session_state["depots_map_df"].copy()
depot_map = dict(zip(depots_map_df["Country"], depots_map_df["Depot"]))
depots_all = sorted(set(depots_map_df["Depot"].tolist() + ["Default"]))
# -----------------------------------------------------
...
# Demand by (Depot, Kit_Type) monthly (integer bottles)
demand_depot_kit = pd.DataFrame(
    0,
    index=pd.to_datetime(study_months_str),
    columns=pd.MultiIndex.from_product([depots_all, kit_types_all], names=["Depot","Kit_Type"])
)
    ...
    for c in country_list:
        ...
        if use_mc and len(active_kits_order) > 0 and n_active > 0:
            ...
            for kt, n_subj in zip(active_kits_order, alloc):
                subj_alloc_active[(m, c, kt)] = int(n_subj)
                if n_subj <= 0:
                    continue
                for k in range(max_offset_local + 1):
                    t = m + k
                    ...
                    if add > 0:
                        ...
                        dep = depot_map.get(c, "Default")
                        demand_depot_kit.loc[row_idx, (dep, kt)] += math.ceil(add * (1.0 + buffer_percent))
        else:
            total_bottles = 0.0
            for k in range(max_offset_local + 1):
                t = m - k
        ...
        if use_mc and len(placebo_kits_order) > 0 and n_pbo > 0:
            ...
            for kt, n_subj in zip(placebo_kits_order, alloc):
                subj_alloc_placebo[(m, c, kt)] = int(n_subj)
                if n_subj <= 0:
                    continue
                for k in range(max_offset_local + 1):
                    t = m + k
            ...
            if total_bottles > 0 and len(placebo_kits_order) > 0:
                ...
                for kt in placebo_kits_order:
                    demand_label_kit.loc[pd.to_datetime(month_lbl), (lg, kt)] += per_kit
        # ---- OLE carryover: roll blinded subjects into matching _OLE kits starting this month ----
        for kt_ole in ole_kits:
            base_act, base_pbo = ole_base_names(kt_ole)
            if not base_act or not base_pbo:
                continue
            n_from_base = int(subj_alloc_active.get((m, c, base_act), 0)) + int(subj_alloc_placebo.get((m, c, base_pbo), 0))
            if n_from_base <= 0:
                continue
            for k2 in range(max_offset_local + 1):
                t2 = m + k2
                if t2 >= len(study_months_str):
                    break
                surv2 = survival_by_country[c][k2] if k2 < len(survival_by_country[c]) else survival_by_country[c][-1]
                bps2 = kit_bottles_ps["Active"].get((k2, kt_ole), 0.0)
                add2 = n_from_base * surv2 * bps2
                if add2 > 0:
                    demand_kit_monthly.iloc[t2][kt_ole] += math.ceil(add2 * (1.0 + buffer_percent))
                    row_idx2 = pd.to_datetime(study_months_str[t2])
                    demand_label_kit.loc[row_idx2, (lg, kt_ole)] += math.ceil(add2 * (1.0 + buffer_percent))
        depot = depot_map.get(c, "Default")
        demand_depot_kit.loc[row_idx2, (depot, kt_ole)] += math.ceil(add2 * (1.0 + buffer_percent))

# Ensure downstream expects integers

# ---------- OLE Attribution (by month, per country) ----------
with st.expander("🔎 OLE attribution (Subjects → OLE, by month & country)", expanded=False):
    ole_attr_rows = []
    for m, month_lbl in enumerate(study_months_str):
        for c in country_list:
            for kt_ole in ole_kits:
                ...
                if not base_act or not base_pbo:
                    continue
                n_from_active  = int(subj_alloc_active.get((m, c, base_act), 0))
                n_from_placebo = int(subj_alloc_placebo.get((m, c, base_pbo), 0))
                n_total = n_from_active + n_from_placebo
                if n_total > 0:
                    ole_attr_rows.append({
                        "Month": pd.to_datetime(month_lbl),
                        "Country": c,
                        "OLE_Kit": kt_ole,
                        "From_Active": n_from_active,
                        "From_Placebo": n_from_placebo,
                        "Total_Subjects_to_OLE": n_total,
                    })

    ole_attr = pd.DataFrame(ole_attr_rows).sort_values(["Month", "Country", "OLE_Kit"]).reset_index(drop=True)

    if ole_attr.empty:
        st.info("No subjects rolled into OLE kits in the current scenario.")
    else:
        st.dataframe(ole_attr, use_container_width=True, hide_index=True)

        st.markdown("**Monthly totals per OLE kit (all countries)**")
        ole_monthly = (
            ole_attr.groupby(["Month", "OLE_Kit"], as_index=False)[["From_Active", "From_Placebo", "Total_Subjects_to_OLE"]]
            .sum()
            .sort_values(["Month", "OLE_Kit"])
        )
        st.dataframe(ole_monthly, use_container_width=True, hide_index=True)

        st.markdown("**Country × OLE kit per month (subjects)**")
        ole_pivot = (
            ole_attr.pivot_table(index="Month", columns=["Country", "OLE_Kit"], values="Total_Subjects_to_OLE", aggfunc="sum", fill_value=0)
            .sort_index(axis=1)
        )
        st.dataframe(ole_pivot, use_container_width=True)

        # Optional: bottle expectation for OLE demand (subjects × OLE visit plan × buffer)
        ole_bottle_rows = []
        for m, month_lbl in enumerate(study_months_str):
            for c in country_list:
                lg = label_map.get(c, "UNSET")
                for kt_ole in ole_kits:
                    ...
                    for k2 in range(max_offset_local + 1):
                        ...
                        surv2 = survival_by_country[c][k2] if k2 < len(survival_by_country[c]) else survival_by_country[c][-1]
                        bps2  = kit_bottles_ps["Active"].get((k2, kt_ole), 0.0)
                        add2  = n_total * surv2 * bps2
                        if add2 > 0:
                            ole_bottle_rows.append({
                                "Month": pd.to_datetime(study_months_str[t2]),
                                "Label_Group": lg,
                                "OLE_Kit": kt_ole,
                                "Bottles": int(math.ceil(add2 * (1.0 + buffer_percent)))
                            })
        ole_bottles_df = pd.DataFrame(ole_bottle_rows)
        if ole_bottles_df.empty:
            st.info("No OLE bottles implied (check OLE visit rows / Qty).")
        else:
            ole_bottles_monthly = (
                ole_bottles_df.groupby(["Month", "OLE_Kit"], as_index=False)["Bottles"].sum()
                .sort_values(["Month", "OLE_Kit"])
            )
            st.dataframe(ole_bottles_monthly, use_container_width=True, hide_index=True)

        import io
        _buf_attr = io.StringIO(); ole_attr.to_csv(_buf_attr, index=False)
        st.download_button("Download OLE Attribution (subjects → OLE)", _buf_attr.getvalue(),
                           file_name="ole_attribution_subjects.csv", mime="text/csv")

        if not ole_bottles_df.empty:
            _buf_b = io.StringIO(); ole_bottles_df.to_csv(_buf_b, index=False)
            st.download_button("Download OLE Bottles (implied)", _buf_b.getvalue(),
                               file_name="ole_attribution_bottles.csv", mime="text/csv")
# ---------------------------------------------------------------------------

demand_depot_kit = demand_depot_kit.astype(int)

with st.expander("Demand by Depot & Kit (monthly)", expanded=False):
    st.dataframe(demand_depot_kit, use_container_width=True)
    if hasattr(demand_depot_kit.columns, "levels"):
        depots = list(demand_depot_kit.columns.levels[0])
    else:
        depots = sorted(set([c for c,_ in demand_depot_kit.columns]))
    depot_tabs = st.tabs([f"Depot: {d}" for d in depots])
    for tab, dep in zip(depot_tabs, depots):
        with tab:
            sub = demand_depot_kit.xs(dep, axis=1, level="Depot")
...
def convert_to_excel(dfs: dict):
    ...
    # guard on per_kit_qty_mode for reproducible export
    try:
        _ = per_kit_qty_mode
    except NameError:
        _ = False
    ...
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        for name, df in dfs.items():
            try:
                df.to_excel(writer, index=True, sheet_name=str(name)[:31])
            except Exception:
                # attempt to coerce non-DataFrame to DataFrame
                try:
                    pd.DataFrame(df).to_excel(writer, index=True, sheet_name=str(name)[:31])
                except Exception:
                    pass
    output.seek(0)
...
excel_bytes = convert_to_excel({
    ...
    "Supply_onhand_by_Kit": supply_onhand_monthly.astype(int),
    "Shortfall_by_Kit": shortfall_monthly.astype(int),
    "Supply_onhand_by_Label_Kit": supply_onhand_labelkit.astype(int),
    "Shortfall_by_Label_Kit": shortfall_labelkit.astype(int),
    "Demand_by_Label_Kit": demand_label_kit.astype(int),
